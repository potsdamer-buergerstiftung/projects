...
const haveFilesAccess = Boolean(
  collectionStore.getCollection("directus_files")
);
...
async function emitValue(
  context: EditorJS.API,
  targetBlock: EditorJS.BlockAPI
) {
...
.bordered {
  padding: var(--input-padding) 4px var(--input-padding)
    calc(var(--input-padding) + 8px) !important;
