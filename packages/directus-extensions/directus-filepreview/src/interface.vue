<template>
	<div class="file-preview">
		<video v-if="['video'].includes(previewFileType.type)" controls :src="src ?? undefined" alt=""
			role="presentation" />
		...
		<iframe v-if="['pdf'].includes(previewFileType.type)" :src="src ?? undefined" frameborder="0"></iframe>
	</div>
</template>
...
const props = withDefaults(defineProps<{ value: string | object | null, fileField: string }>(), { fileField: '', value: null });
...
const src = computed(() => {
	...
	if (previewFileType.value.type) {
		return addTokenToURL((previewFileType.value.type === 'video' ? '' : window.location.origin) + getRootPath() + `assets/${file.value.id}.${previewFileType.value.stuff}`, api);
	}
	return null;
});
