            ...
            <section class="relative h-screen w-full snap-start bg-red-200">
                ...
                <div class="absolute top-1/2 w-full -translate-y-1/2">
                    <div class="container mx-auto px-4">
                        <ul class="flex flex-row space-x-4 mb-2" v-if="project.categories">
                            <li class="text-white font-medium text-lg" v-for="{ project_categories_id: category } in project.categories">{{
                                    category.name
                            }}</li>
                        </ul>
                        ...
                        <h4 class="mt-4 text-lg text-slate-300">{{ project.sub_title }}</h4>
...
const projects = await getItems<any>({
    collection: "projects",
    params: {
        fields: ["title", "sub_title", "image", "id", "categories.project_categories_id.name"],
        sort: "sort",
    },
});
</script>
